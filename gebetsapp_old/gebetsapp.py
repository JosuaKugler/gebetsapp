...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  6 15:57:03 2018
...
import gebetsapp_hilfsklassen

conn = MySQLdb.connect(host="localhost",
                       user="josua",
                       password="Immanueljan1?",
...
class Row(QGridLayout):
    ...
    def beten(self):
        self.b = gebetsapp_hilfsklassen.BeteFenster(self.ID)
...
class addPersonDialog(QMainWindow):
    ...
    def init(self):
        ...
        w = QWidget()
        self.vorname = gebetsapp_hilfsklassen.InputLayout("Vorname:*")
        if self.vornamentext:
            self.vorname.Input.setText(self.vornamentext)
        self.nachname = gebetsapp_hilfsklassen.InputLayout("Nachname:")
        if self.nachnamentext:
            self.nachname.Input.setText(self.nachnamentext)
        self.geburtsdatum = gebetsapp_hilfsklassen.InputLayout("Geburtsdatum:",
                                                               PlaceHolderText="JJJJ-MM-TT",
                                                               InputMask="9999-99-99")
        if self.geburtsdatumstext!=None:
            self.geburtsdatum.Input.setText(str(self.geburtsdatumstext))
        self.bitte = gebetsapp_hilfsklassen.TextLayout("Bitte")
        self.danke = gebetsapp_hilfsklassen.TextLayout("Danke")
...
class Notification(QMainWindow):
    ...
    def click(self):
        self.destroy()
        if self.ID != None:
            w.a = gebetsapp_hilfsklassen.BeteFenster(self.ID)
        else:
...
class App(QTabWidget):
    ...
    def getrandomid(self):
        with shelve.open("oldidlist","wb") as oldidlist:
            self.oldidlist = oldidlist["liste"]
            c.execute("""SELECT person_id FROM personen""")
            a = c.fetchall()
            idlist=[]
            for i in a:
            ...
            if len(workidlist)==0:
                ...
                oldidlist["liste"]=self.oldidlist
                oldidlist.sync()
                return ID
